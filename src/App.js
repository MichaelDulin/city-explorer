...
class App extends React.Component {
  ...
  handleWeather = async () => {
    this.handleMovies();
    let weatherUrl = `${process.env.REACT_APP_SERVER}/weather?lat=${this.state.lat}&lon=${this.state.long}`;
    let getWeather;
    try{
  ...
  handleMovies = async () => {
    let movieUrl = `${process.env.REACT_APP_SERVER}/movie?city=${this.state.cityName}`;
    let getMovies;
    try{
  ...
  render() {
    ...
    let forecast = this.state.weatherData.map((weather, index) => {
      return <Weather key={index} date={weather.date} description={weather.description} />;
    });
